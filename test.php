		...
		function check_test($test_file)
		{
			...
			foreach($test_file as $key => $item){
				$questions++;
           		...
           		echo 'Your answer ' . $item['answers'][$_POST['answer' . $key]] . '<br>';
           		echo 'The right answer: ' . $item['answers'][$item['correct_answer']] . '<br>';
           		echo '</div>';
           		echo '<hr>';
			}
			echo '<p style="font-weight: bold;">All correct answers: ' . $i . 'from' . $questions . '</p>';
		}
	}
?>
...
<body>
	...
	<a href="<?php echo isset($_POST['check-test']) ? $_SERVER['HTTP_REFERER'] : 'list.php' ?>">Go back</a><br>
	<a href="index.php">Load the test</a>
	...
	<?php if(isset($_GET['number']) && !isset($_POST['check-test'])): ?>
		<form method="post">
			...
			<?php endforeach ?>
			<input type="submit" name="check-test" value="Check">
		</form> 
	<?php endif?>
	<div>
